...
import type { GroqWhisperConfig, STTProvider } from '../interface.js';
...
export class GroqWhisperSTTProvider extends EventEmitter implements STTProvider {
  readonly name = 'groq-whisper';

  private connected = false;
  private _config: GroqWhisperConfig | null = null;
  private options: Required<GroqWhisperOptions>;
  ...
  private static readonly DEFAULT_INTERIM_MS = 500;
  private static readonly DEFAULT_MIN_AUDIO = 1600;

  constructor(options: GroqWhisperOptions = {}) {
    super();
    this.options = {
      apiUrl: options.apiUrl || 'https://api.groq.com/openai/v1/audio/transcriptions',
      silenceTimeoutMs: options.silenceTimeoutMs ?? GroqWhisperSTTProvider.DEFAULT_SILENCE_MS,
      interimIntervalMs: options.interimIntervalMs ?? GroqWhisperSTTProvider.DEFAULT_INTERIM_MS,
      minAudioSizeBytes: options.minAudioSizeBytes ?? GroqWhisperSTTProvider.DEFAULT_MIN_AUDIO,
    };
  }

  async connect(config: GroqWhisperConfig): Promise<void> {
    this._config = config;
...
export function createGroqWhisperSTTProvider(options?: GroqWhisperOptions): GroqWhisperSTTProvider {
  return new GroqWhisperSTTProvider(options);
}
