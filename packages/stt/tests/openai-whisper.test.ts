...
describe('OpenAIWhisperSTTProvider', () => {
  let provider: any;
  let OpenAIWhisperSTTProvider: any;
  ...
  beforeEach(async () => {
    const mod = await import('../src/adapters/openai-whisper.js');
    OpenAIWhisperSTTProvider = mod.OpenAIWhisperSTTProvider;
    provider = new OpenAIWhisperSTTProvider();
  });
  ...
  describe('connect', () => {
    ...
    it('should throw without API key', async () => {
      await expect(
        provider.connect({ provider: 'openai-whisper', sampleRate: 16000 }),
  ...
  describe('streamAudio', () => {
    ...
    it('should accumulate audio chunks when connected', async () => {
      await provider.connect({
        provider: 'openai-whisper',
        apiKey: 'test-key',
        sampleRate: 16000,
  ...
  describe('silence detection', () => {
    ...
    it('should reset silence timer on each audio chunk', async () => {
      vi.useFakeTimers();
      await provider.connect({
        provider: 'openai-whisper',
        apiKey: 'test-key',
      ...
      provider.streamAudio(validChunk);
      const timerBefore = provider.silenceTimer;

      provider.streamAudio(validChunk);
      const timerAfter = provider.silenceTimer;
  ...
  describe('close', () => {
    it('should clean up state', async () => {
      ...
      expect(provider.isConnected()).toBe(false);
      expect(provider.audioBuffer).toEqual([]);
    });
  });
  ...
  describe('callback registration', () => {
    ...
    it('should register error callback', () => {
      const cb = vi.fn();
      provider.onError(cb);
      provider.emit('error', new Error('test error'));
      expect(cb).toHaveBeenCalled();
    });
  });
});
