...
class InlineTTSProvider implements TTSProvider {
  ...
  constructor(
    name: string,
    opts?: { failSynthesize?: boolean; failMidSynthesis?: boolean },
  ) {
    this.name = name;
...
describe('CompositeSTTProvider', () => {
  ...
  it('should construct with provider list from factory', () => {
    ...
    sttMap = new Map([['p1', stt1], ['p2', stt2]]);
  ...
  describe('streamAudio()', () => {
    ...
    it('should trigger failover when active provider errors on streamAudio', async () => {
      ...
      const originalStream = errorProvider.streamAudio.bind(errorProvider);
  ...
  describe('close()', () => {
    ...
    it('should silently ignore provider close errors', async () => {
      ...
      const failMap = new Map([['p1', closeFail1], ['p2', closeFail2]]);
      const comp = new CompositeSTTProvider(
        { providers: ['p1', 'p2'] },
        (n) => createProvider(n, failMap),
      );
      await comp.connect({});
...
describe('CompositeTTSProvider', () => {
  ...
  it('should construct with provider list from factory', () => {
    ...
    ttsMap = new Map([['p1', tts1], ['p2', tts2]]);
  ...
  describe('connect()', () => {
    ...
    it('should throw when all providers fail to connect', async () => {
      ...
      const originalConnect = p1.connect.bind(p1);
